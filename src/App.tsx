...
function AppInner() {
  ...
  const buildQueue = useCallback((type: string, goal: number): Question[] => {
    ...
    if (type === 'review') return cards.filter(c => { const s = getCardState(c.id); return s && s.nextReview <= Date.now() })
    ...
    if (type === 'review-due') {
      return cards.filter(c => {
        ...
        return !!s && s.reviewCount > 0 && s.nextReview <= Date.now()
      })
    }
    ...
    const reviewIds = new Set(
      cards
        .filter(c => { const s = getCardState(c.id); return !!s && s.reviewCount > 0 && s.nextReview <= Date.now() })
        .map(c => c.id)
    )
